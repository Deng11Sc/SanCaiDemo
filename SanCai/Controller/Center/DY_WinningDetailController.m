...
- (void)viewDidLoad {
    ...
    scrollView.contentSize = CGSizeMake(DY_Width, DY_SCREEN_MAX);
...
-(UILabel *)titleLabel
{
    ...
    if (!_titleLabel) {
        ...
        [titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.width.mas_lessThanOrEqualTo(DY_Width-16);
        }];
...
-(UILabel *)descLabel
{
    ...
    if (!_descLabel) {
        ...
        [descLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.width.mas_lessThanOrEqualTo(DY_Width-24);
    ...
    [NSString server_getScoresModelWithUserId:SELF_USER_ID blk:^(DY_ScoresModel *scoresModel) {
        if ([scoresModel.scores integerValue] >= [self.winModel.priceScores integerValue]) {
            ...
            addrVC.selectAddressBlk = ^(NSString *address) {
                ...
                ///减少积分
                [NSString server_saveScoresWithUserId:SELF_USER_ID objId:scoresModel.objId changedNumber:-[self.winModel.priceScores integerValue]];
...
- (void)setIsLandscape:(BOOL)isLandscape {
    ...
    [_reciveAddressLabel mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(@43);
        make.left.mas_equalTo(12);
        make.width.mas_equalTo(DY_Width-24);
    }];
    ...
    if (isLandscape) {
        _scrollView.contentSize = CGSizeMake(DY_Width, DY_SCREEN_MAX);
    } else {
        _scrollView.contentSize = CGSizeMake(DY_Width, DY_Height);
    }
